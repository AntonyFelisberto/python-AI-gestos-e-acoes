...
imagem = cv2.imread(f'arquivos\\imagens\\body\\multiple\\multiple_1.jpeg')

# carregando o modelo COCO
# arquitetura da rede neural
arquivoProto = f'arquivos\\pose\\body\\coco\\pose_deploy_linevec.prototxt'
# pesos do modelo
arquivoModelo = f'arquivos\\pose\\body\\coco\\pose_iter_440000.caffemodel'
# este modleo tem 19 pontos de reconhecimento
nPontos = 18
# pares de poses
...
POSE_PARES = [[1, 2], [1, 5], [2, 3], [3, 4], [5, 6], [6, 7],[1, 8], [8, 9], [9, 10], [1, 11], [11, 12], [12, 13],
              [1, 0], [0, 14], [14, 16], [0, 15], [15, 17],[2, 17], [5, 16]]

# indice do mapa de afinidade correspondente ao POSE_PARES
# para POSE_PARES(1,2), as partes de afinidade estão localizadas nos índices (31,32) de saída. Similarmente, (1,5) -> (39,40) e assIm por diante.
# Seção 3 - Aula 18.
...
cores = np.random.randint(0,255,(100,3))
...
for i in range(nPontos):
    for j in range(len(pontosChave_detectados[i])):
        cv2.circle(imagemCopia, pontosChave_detectados[i][j][0:2], 4, cores[i].tolist(), -1, cv2.LINE_AA)
...
for i in range(17):
    for n in range(len(pontosChavePessoa)):
        ...
        cv2.line(imagemCopia, (B[0], A[0]), (B[1], A[1]), cores[i].tolist(), 2, cv2.COLOR_BGR2RGB)

plt.imshow(cv2.line(imagemCopia, (B[0], A[0]), (B[1], A[1]), cores[i].tolist(), 2, cv2.COLOR_BGR5552GRAY))
plt.show()
